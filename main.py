...
def draw_bg():
    ...
    for x in range(5):
        ...
        screen.blit(sky_img, ((x * width) - bg_scroll * 0.0001,0))
...
class Soldier(pygame.sprite.Sprite):
    def __init__(self, char_type, x, y, scale, speed, ammo, grenades):
        ...
        self.alive = True
        # scaling maane size badhana
        self.shoot_cooldown = 0
        self.speed = speed
        self.ammo = ammo
        self.stsrt_ammo = ammo
        self.y_vel = 0
        self.flip = False
        ...
        self.vision = pygame.Rect(5,5, 150, 20)
    ...
    def move(self, moving_left, moving_right):
        ...
        for tile in world.obstacle_list:
            # checking collisions in x direction
            if tile[1].colliderect(self.rect.x + dx, self.rect.y, self.width, self.height):
                dx = 0
                #if ai has hit the wallthen make it turn around
        ...
        if self.char_type == 'player':
            if (self.rect.right > SCREEN_WIDTH - SCREEN_THRESH and bg_scroll<(world.level_length*TILE_SIZE)) or (self.rect.left < SCREEN_THRESH and bg_scroll>abs(dx)):
                self.rect.x -= dx
                screen_scroll = -dx
    ...
    def update_animation(self):
        ...
        # update imag depending on curent frame
        self.image = self.animation_list[self.action][self.frame_index]
        # updatemage depending on current frame
    ...
    def update_action(self, new_action):
        # chechk if the new action is different to the prevoius action
...
class ItemBox(pygame.sprite.Sprite):
    ...
    def update(self):
        self.rect.x += screen_scroll
        # check if player has pickedt the box
...
class Explosion(pygame.sprite.Sprite):
    def __init__(self, x, y, scale):
        ...
        for img in range(1, 6):
            img = pygame.image.load(f'img/explosion/exp{img}.png')
            img = pygame.transform.scale(
                img, (int(img.get_width() * scale), int(img.get_height() * scale)))
...
while run:
    ...
    if start_game ==False:
        ...
        # display ammo
        draw_text('AMMO:', font, WHITE, 10, 25)
        for x in range(player.ammo):
            screen.blit(bullet_img, (130 + (x * 10), 45))
        player.update()
        player.draw()
        for enemy in enemy_group:
            enemy.ai()
